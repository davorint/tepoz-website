...
interface EnhancedMapSectionProductionProps {
  lang: Locale
}

export default function EnhancedMapSectionProduction({ lang }: EnhancedMapSectionProductionProps) {
  ...
  // Use custom hook for cleaner separation of concerns
  const { filteredPlaces, totalPlaces } = useMapPlaces({
    searchQuery,
    selectedCategory,
    sortBy,
    lang
  ...
  // DEBUG: Log production data like Step 4
  console.log('🗺️ PRODUCTION MAP DATA:', {
    ...
    sortBy,
    lang,
    samplePlaces: filteredPlaces.slice(0, 3).map(p => ({ id: p.id, name: p.name, coords: p.coordinates }))
  })

  // Clear selection if selected place is no longer in filtered results
  useEffect(() => {
    if (selectedPlace && !filteredPlaces.find(p => p.id === selectedPlace)) {
  ...
  // Memoized handlers for better performance - UPDATED FROM STEP 4
  const handlePlaceSelect = useCallback((placeId: string) => {
    console.log('🗺️ PRODUCTION: Place selected via click:', placeId)
    try {
      setSelectedPlace(placeId)
      
      // Find place in filtered list for better accuracy
      const place = filteredPlaces.find(p => p.id === placeId)
      if (place) {
        // Scroll to sidebar element (like Step 4)
        const element = document.getElementById(`place-${placeId}`)
        if (element) {
          element.scrollIntoView({ behavior: 'smooth', block: 'center' })
        }
        
        console.log('✅ PRODUCTION: Place found and selected:', place.name)
      } else {
        console.warn('❌ PRODUCTION: Place not found in filtered list:', placeId)
        console.warn('Available place IDs:', filteredPlaces.map(p => p.id))
      }
    } catch (error) {
      console.warn('❌ PRODUCTION: Error selecting place:', error)
    }
  }, [filteredPlaces])

  const handleCategoryChange = useCallback((categoryId: string) => {
    // Validate category exists
    const validCategories = ['all', 'restaurant', 'hotel', 'attraction', 'cafe', 'shopping', 'culture', 'bar']
  ...
  const handleKeyPress = useCallback((e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      // Trigger search - already handled by real-time filtering
      e.preventDefault()
    }
    if (e.key === 'Escape') {
      setSearchQuery('')
  ...
  // Log map rendering details (after handlers are defined)
  console.log('🎯 PRODUCTION: Rendering UnifiedMap with:', {
    filteredPlacesCount: filteredPlaces.length,
    selectedPlaceId: selectedPlace,
    hasPlaceSelect: !!handlePlaceSelect,
    firstPlaceCoords: filteredPlaces[0]?.coordinates,
    firstPlaceId: filteredPlaces[0]?.id
  })
