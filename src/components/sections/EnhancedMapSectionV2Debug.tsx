...
interface EnhancedMapSectionV2DebugProps {
  lang: Locale
}

export default function EnhancedMapSectionV2Debug({ lang }: EnhancedMapSectionV2DebugProps) {
  ...
  console.log('🔄 EnhancedMapSection render:', {
    searchQuery,
    selectedCategory,
    sortBy,
  ...
  useEffect(() => {
    console.log('📱 Filter effect triggered:', { searchQuery, selectedCategory, sortBy })
    
    let places = [...tepoztlanPlaces]
    console.log('📍 Starting with places count:', places.length)
    
    // Filter by category FIRST
    if (selectedCategory !== 'all') {
      places = places.filter(p => p.category === selectedCategory)
      console.log('🏷️ After category filter:', places.length, 'places for category:', selectedCategory)
    }
    ...
    if (searchQuery.trim()) {
      ...
      places = places.filter(place => {
        ...
        const matchesNameEn = place.nameEn?.toLowerCase().includes(searchTerm) || false
        const matchesDescriptionEn = place.descriptionEn?.toLowerCase().includes(searchTerm) || false
        const matchesTags = place.tags?.some(tag => tag.toLowerCase().includes(searchTerm)) || false
        const matchesCategory = place.category.includes(searchTerm)
        
        return matchesName || matchesDescription || matchesNameEn || matchesDescriptionEn || matchesTags || matchesCategory
      })
      console.log('🔍 After search filter:', places.length, 'places for query:', searchQuery)
    }
    ...
    console.log('📋 Places list:', places.map(p => p.name))
  ...
  const handlePlaceSelect = (placeId: string) => {
    ...
    if (place) {
      ...
      if (element) {
        ...
        console.log('📜 Scrolled to place in sidebar')
      } else {
        console.log('❌ Place element not found in sidebar')
      }
    }
  }
  ...
  const handleSearchChange = (value: string) => {
    ...
    setSelectedPlace(null) // Clear selection when search changes
  }

  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      console.log('⌨️ Enter pressed, search query:', searchQuery)
    }
  }
                ...
                <Card className="border-0 bg-white/5 backdrop-blur-xl">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-white/40 text-sm mt-2">
                      {lang === 'es' 
                        ? `Buscar: "${searchQuery}" en categoría: ${selectedCategory}`
                        : `Search: "${searchQuery}" in category: ${selectedCategory}`
                      }
                    </p>
                  </CardContent>
                </Card>
