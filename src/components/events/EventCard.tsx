...
export default function EventCard({ 
  ...
  const getCategoryColor = () => {
    const categoryColors = {
      'festival': 'from-purple-400 to-pink-400',
      ...
      'workshop': 'from-indigo-400 to-purple-400',
    ...
    >
      ...
      <Card className={`relative bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl hover:shadow-3xl overflow-hidden transform-gpu transition-all duration-500 group-hover:scale-[1.02] group-hover:bg-white/15 ${
        ...
        <CardContent className={`p-6 ${viewMode === 'list' ? 'flex-1' : ''}`}>
          ...
          {/* Price */}
          <div className="mb-4 p-3 bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-lg border border-purple-400/30 backdrop-blur-sm">
          ...
          {event.highlights && event.highlights[locale].length > 0 && (
            <div className="flex flex-wrap gap-1 mb-4">
              {EventService.getEventHighlights(event, locale).slice(0, 2).map((highlight, idx) => (
                <Badge key={idx} variant="outline" className={`text-xs border-purple-400/30 text-purple-300 bg-gradient-to-r ${getCategoryColor()}/10`}>
                  ✨ {highlight}
                </Badge>
              ))}
          ...
          <div className="flex gap-2">
            <Button 
              variant="default" 
              size="sm"
              className={`flex-1 bg-gradient-to-r ${getCategoryColor()} hover:opacity-90 text-white border-0 shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-300`}
            >
