...
export default function WorkingMap({ 
  ...
  const isFullscreenSupported = useCallback(() => {
    return !!(
      ...
      (document as Document & { webkitFullscreenEnabled?: boolean, mozFullScreenEnabled?: boolean, msFullscreenEnabled?: boolean, webkitRequestFullscreen?: () => Promise<void>, mozRequestFullScreen?: () => Promise<void>, msRequestFullscreen?: () => Promise<void>, webkitExitFullscreen?: () => Promise<void>, mozCancelFullScreen?: () => Promise<void>, msExitFullscreen?: () => Promise<void>, webkitFullscreenElement?: Element, mozFullScreenElement?: Element, msFullscreenElement?: Element }).webkitFullscreenEnabled ||
  ...
  useEffect(() => {
    ...
    console.log('🗺️ Initializing map with proper CSS...')
  ...
  useEffect(() => {
    ...
    filteredPlaces.forEach((place) => {
      try {
        ...
        // Create popup
        const popupContent = `
          <div style="min-width: 220px; font-family: system-ui, sans-serif; padding: 8px;">
    ...
    >
      ...
      <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm px-3 py-2 rounded-lg text-sm z-10 border shadow-lg">
        ...
        <div className="text-xs text-gray-500 mt-1">
          {isFullscreen ? '🖥️ Fullscreen' : '🖼️ Window'} | CSS: ✅
        </div>
      </div>
